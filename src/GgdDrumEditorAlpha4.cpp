...
public:
    ...
    void paint(juce::Graphics& g) override
    {
        ...
        // Timeline ruler background and alternating bar wash.
        g.setColour(c(panel));
        ...
        // Ruler tick hierarchy. Bars occupy the full ruler, beats are long,
        // sixteenths are short, and thirty-seconds only appear in detail mode.
        ...
        for (const auto& item : layout)
        {
            if (item.header)
            {
                ...
                // Drum-family boundaries should read before individual rows.
            ...
            // Vertical hierarchy inside the actual drum rows.
            for (int step = 0; step <= numSteps; ++step)
            {
                const float x = xForStorageStepF(step);
                const bool isBar = step % stepsPerBar == 0;
            ...
            {
                float x0 = xForStorageStepF(hoverStep);
            ...
            for (int step = 0; step < numSteps; ++step)
            {
                ...
                {
                    // Existing x2 timing is exactly two adjacent 1/32 hits.
                ...
                {
                    // +50% of a 1/16 cell lands exactly on its second 1/32.
                    drawHit(slotStart + slotWidth * 0.75f, slotWidth * 0.5f);
                    spatial32 = true;
                }
                ...
                // Exact 1/32 placements are already communicated spatially.
                ...
                // In detail mode an x2 is drawn as two actual hits. x3/x4 remain
                // explicit retrigger gestures and retain their label.
        ...
        if (playPosition >= 0 && playPosition < numSteps)
        {
            ...
            g.setColour(c(accent).withAlpha(0.085f));
    ...
    void mouseDown(const juce::MouseEvent& e) override
    {
        ...
        {
            // Explicit rolls remain cell-level. In 1/32 mode x2 will be drawn as
            // the two concrete half-step hits that it actually produces.
...
private:
    ...
    void rebuildLayout()
    {
        ...
        auto* layer = processor.mData.getUISeqData()->getLayer(0);
        const int patternRight = static_cast<int>(std::ceil(xForStorageStepF(layer->getNumSteps())));
                       ...
                       bool& secondHalf) const
    {
        ...
        if (detail32Active())
        {
            const float localX = p.x - xForStorageStepF(step);
    ...
    void setDisplayCell(int canonicalRow, int step, bool secondHalf, bool on)
    {
        ...
        if (on)
        {
            ...
            if (!anyOn)
            {
                ...
                layer->setOffset(storageRow, step, static_cast<int8_t>(secondHalf ? 50 : 0), pattern);
            }
            else
            {
                // The other half already exists. x2 produces exactly the two
                // half-step events we need while preserving one semantic cell.
                layer->setLength(storageRow, step, -1, pattern);
                layer->setOffset(storageRow, step, 0, pattern);
            }
            return;
        ...
        {
            layer->setLength(storageRow, step, 0, pattern);
            layer->setOffset(storageRow, step, static_cast<int8_t>(secondHalf ? 0 : 50), pattern);
        }
        else
        {
            setCell(canonicalRow, step, false);
    ...
    : juce::AudioProcessorEditor(&p), processor(p)
{
    ...
    zoomSlider.setTooltip("Absolute timeline zoom. 125% is the default. At 350% and above the editor exposes a 1/32 detail grid.");
    ...
    hintLabel.setText(
        ...
        "Timing: Alt-drag  •  Zoom: Ctrl+wheel  •  Ghost: Ctrl-click  •  Roll: Ctrl+Shift-click",
...
void GgdDrumEditor::resized()
{
    ...
    zoomValueLabel.setBounds(second.removeFromRight(84));
