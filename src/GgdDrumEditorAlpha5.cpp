...
bool isPluginUndoKey(const juce::KeyPress& key)
{
    ...
    return !mods.isCtrlDown() && !mods.isCommandDown() && !mods.isAltDown()
        && !mods.isShiftDown() && key.getKeyCode() == 'U';
}
...
public:
    ...
    bool keyPressed(const juce::KeyPress& key) override
    {
        ...
        if ((isHostUndoKey(key) || isPluginUndoKey(key)) && undoCallback)
        {
            undoCallback();
            return true;
        }
...
private:
    ...
    bool selectionTargetsAreValid(int deltaSteps, int deltaRows) const
    {
        if (selection.empty())
            return false;
        ...
        for (const auto& ref : selection)
        {
            ...
            if (cellOccupied(targetRow, targetStep)
                && !isSelected(targetRow, targetStep))
                return false;
        }
    ...
    bool moveSelection(int deltaSteps, int deltaRows)
    {
        ...
        if (!selectionTargetsAreValid(deltaSteps, deltaRows))
            return false;
    ...
    void duplicateSelection()
    {
        ...
        const int delta = juce::jmax(1, maxStep - minStep + 1);

        auto* layer = processor.mData.getUISeqData()->getLayer(0);
        const int numSteps = layer->getNumSteps();
        for (const auto& ref : selection)
        {
            const int target = ref.step + delta;
            if (target >= numSteps || (cellOccupied(ref.row, target) && !isSelected(ref.row, target)))
                return;
        }
        ...
        for (const auto& ref : selection)
        {
            ...
            const int target = ref.step + delta;
            writeCellState(state, ref.row, target);
            duplicated.push_back({ ref.row, target });
        }
    ...
    {
        if (!selectionTargetsAreValid(moveDeltaSteps, moveDeltaRows))
        {
    ...
    : juce::AudioProcessorEditor(&p), processor(p)
{
    ...
    grid = std::make_unique<GgdDrumGrid>(
        ...
        [this](GgdDrumGrid::ToolMode mode)
        {
            ...
            hintLabel.setText(
                draw
                    ? "Draw: click/drag  •  Erase: right-drag  •  Velocity: Shift-drag or Alt+wheel  •  Timing: Alt-drag  •  D/S switch tools  •  U undo"
                    : "Select: click  •  Add: Shift-click  •  Box: drag empty space  •  Move: drag selection / arrows  •  Delete  •  Ctrl+D duplicate  •  U undo",
    ...
    undoButton.setTooltip("Undo the most recent sequence edit. U is the plugin-local shortcut; Bitwig may intercept Ctrl+Z before the plugin sees it.");
...
{
    if (isHostUndoKey(key) || isPluginUndoKey(key))
    {
        performUndo();
        return true;
    }
