...
public:
    ...
    bool handleShortcut(const juce::KeyPress& key)
    {
        ...
        if (!mods.isCtrlDown() && !mods.isCommandDown() && !mods.isAltDown()
            && !mods.isShiftDown())
        {
            if (keyMatchesLetter(key, 'D'))
            {
                setToolMode(ToolMode::draw);
                return true;
            }
            if (keyMatchesLetter(key, 'S'))
            {
    ...
    void pollFallbackShortcuts(bool active)
    {
        ...
        const bool plain = !mods.isCtrlDown() && !mods.isCommandDown()
            && !mods.isAltDown() && !mods.isShiftDown();
        if (!plain)
            return;
        ...
        [this](GgdDrumGrid::ToolMode mode)
        {
            ...
            hintLabel.setText(
                draw
                    ...
                    : "Select: click | Add: Shift-click | Box: drag empty space | Move: drag/arrows | Copy: Alt+drag | Delete | U undo",
...
{
    if (grid != nullptr && grid->handleShortcut(key))
        return true;
...
void GgdDrumEditor::resized()
{
    ...
    auto actions = second.removeFromRight(118);
