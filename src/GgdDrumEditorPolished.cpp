...
public:
    ...
    void setZoomScale(float newScale)
    {
        ...
        const float anchorInViewport = static_cast<float>(nameWidth)
                                     + (static_cast<float>(view.getWidth() - nameWidth) * 0.5f);
        const float anchorInContent = static_cast<float>(viewport->getViewPositionX())
                                    + anchorInViewport;
    ...
    void paint(juce::Graphics& g) override
    {
        ...
        // Timeline ruler.
        g.setColour(c(panel));
        g.fillRect(gridLeft, 0, juce::jmax(0, getWidth() - gridLeft), rulerHeight);
        ...
        for (int barStart = 0; barStart < numSteps; barStart += stepsPerBar)
        {
            ...
            if ((bar & 1) != 0)
            {
                g.setColour(c(panelRaised).withAlpha(0.28f));
        ...
        for (const auto& item : layout)
        {
            if (item.header)
            {
                ...
                g.fillRect(gridLeft, item.y, juce::jmax(0, getWidth() - gridLeft), item.height);
            ...
            for (int step = 0; step < numSteps; ++step)
            {
                ...
                const bool alignedToVisibleGrid = (step % snap) == 0;

                const float slotStart = xForStorageStepF(step);
                const float slotEnd = alignedToVisibleGrid
                    ? xForStorageStepF(juce::jmin(numSteps, step + snap))
                    : xForStorageStepF(juce::jmin(numSteps, step + 1));
                ...
                const float hitW = juce::jlimit(7.0f, 28.0f, slotWidth * 0.42f);
        ...
        // Sticky articulation lane. It is deliberately painted last so content
        // slides underneath a fixed visual column without requiring dummy tail space.
        g.setColour(c(panel));
        ...
        g.setColour(c(muted));
        g.setFont(10.0f);
        const auto zoomCaption = (zoomScale <= 1.015f ? "FIT  •  " : juce::String(zoomScale, 2) + "x  •  ")
                               + resolutionText() + " GRID";
                        ...
                        const juce::MouseWheelDetails& wheel) override
    {
        ...
        if (e.mods.isCtrlDown() || e.mods.isCommandDown())
        {
            ...
            // Continuous scaling makes trackpads and high-resolution wheels feel
            // natural. Resolution changes only at musical thresholds.
            const float multiplier = std::pow(2.0f, primaryDelta * 2.2f);
...
private:
    ...
    static constexpr float minZoomScale = 1.0f;
    static constexpr float maxZoomScale = 3.0f;
    ...
    int timelineViewportWidth() const
    {
        if (auto* viewport = findParentComponentOfClass<juce::Viewport>())
            return juce::jmax(240, viewport->getViewArea().getWidth() - nameWidth);
        return 760;
    }

    float fittedStorageStepWidth() const
    {
        auto* layer = processor.mData.getUISeqData()->getLayer(0);
        const int numSteps = juce::jmax(1, layer->getNumSteps());
        return static_cast<float>(timelineViewportWidth()) / static_cast<float>(numSteps);
    }

    float storageStepWidth() const
    {
        return juce::jmax(1.0f, fittedStorageStepWidth() * zoomScale);
    }
    ...
    void applyZoom(float requestedScale,
                   float anchorInContent,
                   float anchorInViewport)
    {
        const float newScale = juce::jlimit(minZoomScale, maxZoomScale, requestedScale);
        if (std::abs(newScale - zoomScale) < 0.0005f)
            return;

        auto* viewport = findParentComponentOfClass<juce::Viewport>();
        const float oldStepWidth = storageStepWidth();
        float storageCoordinate = 0.0f;

        if (anchorInContent >= 0.0f && oldStepWidth > 0.0f)
            storageCoordinate = (anchorInContent - static_cast<float>(nameWidth)) / oldStepWidth;
        ...
        if (viewport != nullptr)
        {
            int newViewX = viewport->getViewPositionX();
            if (anchorInContent >= 0.0f && anchorInViewport >= 0.0f)
            {
                const float newAnchor = static_cast<float>(nameWidth)
                                      + storageCoordinate * storageStepWidth();
                newViewX = static_cast<int>(std::round(newAnchor - anchorInViewport));
            }
            else if (zoomScale <= 1.015f)
            {
                newViewX = 0;
            }

            viewport->setViewPosition(juce::jmax(0, newViewX), viewport->getViewPositionY());
        }
    ...
    void rebuildLayout()
    {
        ...
        auto* layer = processor.mData.getUISeqData()->getLayer(0);
        const int timelineWidth = timelineViewportWidth();
        const int fittedWidth = nameWidth + timelineWidth;
        const int scaledWidth = static_cast<int>(
            std::ceil(static_cast<float>(nameWidth) + layer->getNumSteps() * storageStepWidth()));
        setSize(juce::jmax(fittedWidth, scaledWidth), juce::jmax(320, y + 12));
    }
    ...
    : juce::AudioProcessorEditor(&p), processor(p)
{
    ...
    zoomSlider.setRange(1.0, 3.0, 0.0);
    zoomSlider.setValue(1.0, juce::dontSendNotification);
    zoomSlider.setDoubleClickReturnValue(true, 1.0);
    zoomSlider.setTooltip("Smooth timeline zoom. Ctrl+wheel zooms under the mouse.");
    ...
    zoomValueLabel.setText("FIT  •  1/4", juce::dontSendNotification);
    ...
    fitZoomButton.setTooltip("Fit the complete pattern to the editor width");
...
void GgdDrumEditor::paint(juce::Graphics& g)
{
    ...
    // Quiet separators keep the two control rows readable without turning the
    // header into a dashboard of boxes.
...
void GgdDrumEditor::resized()
{
    ...
    fitZoomButton.setBounds(second.removeFromRight(43).reduced(0, 5));
    second.removeFromRight(5);
    zoomValueLabel.setBounds(second.removeFromRight(80));
...
void GgdDrumEditor::refreshZoomControls(float scale)
{
    ...
    if (scale <= 1.015f)
        ...
        prefix = juce::String(scale, 2) + "x";

    juce::String resolution = "1/4";
    if (scale >= 2.28f)
        resolution = "1/16";
    else if (scale >= 1.42f)
        resolution = "1/8";

    zoomValueLabel.setText(prefix + "  •  " + resolution,
                           juce::dontSendNotification);
}
