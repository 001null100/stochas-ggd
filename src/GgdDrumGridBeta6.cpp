...
// Beta 6 keeps the proven event-editing implementation, while replacing the
// presentation and mouse paths that need cell-centred hit visuals, cell-based
// drawing, resolution-independent erasing and articulation-row audition.
...
void GgdDrumGrid::setPlayheadPresentation(bool smoothing, bool glow)
{
    smoothPlayhead = smoothing;
    playheadGlow = glow;
    repaint();
}

float GgdDrumGrid::cellWidthPixels() const
{
    return static_cast<float>(snapTicks) / GGD_EVENT_PPQ
         * pixelsPerQuarter * zoomScale;
}

float GgdDrumGrid::visualHitCenterX(int tick) const
{
    return xForTick(static_cast<float>(tick)) + cellWidthPixels() * 0.5f;
}

int GgdDrumGrid::cellTickForX(float x) const
{
    const int length = patternLengthTicks();
    if (length <= 0)
        return 0;

    const float raw = juce::jmax(0.0f, tickForX(x));
    const int cell = static_cast<int>(std::floor(raw / static_cast<float>(snapTicks))) * snapTicks;
    const int lastCell = juce::jmax(0, ((length - 1) / snapTicks) * snapTicks);
    return juce::jlimit(0, lastCell, cell);
}
...
void GgdDrumGrid::setPlayPosition(int stepPosition)
{
    ...
    if (stepPosition != playStepPosition)
    {
        ...
        const double oldStepMs = juce::jmax(1.0, playStepMs);
        const double elapsedSinceVisualBoundary = now - playStepStartMs;
        const double predictedAdvanceSteps = juce::jlimit(
            -0.20,
            static_cast<double>(delta) + 0.80,
            elapsedSinceVisualBoundary / oldStepMs);

        const double elapsedNotification = now - lastPlayNotificationMs;
        const double observed = elapsedNotification / static_cast<double>(delta);
        if (observed >= 8.0 && observed <= 2000.0)
        {
            ...
            {
                // A lighter correction than Beta 5 makes callback jitter much
                // less visible while still following real tempo changes quickly.
                playStepMs = playStepMs * 0.84 + observed * 0.16;
            }
        }

        // Preserve most of the position we were already drawing when the host's
        // next coarse 1/16 notification arrives. Pull only partway toward the
        // reported boundary, rather than snapping backwards/forwards every step.
        const double phaseErrorSteps = static_cast<double>(delta) - predictedAdvanceSteps;
        const double correctedAdvanceSteps = predictedAdvanceSteps + phaseErrorSteps * 0.34;
        const double fractionIntoNewStep = juce::jlimit(
            -0.12, 0.72, correctedAdvanceSteps - static_cast<double>(delta));

        playStepPosition = stepPosition;
        playStepStartMs = now - fractionIntoNewStep * juce::jmax(1.0, playStepMs);
        lastPlayNotificationMs = now;
    }
...
float GgdDrumGrid::interpolatedPlayTick() const
{
    ...
    if (smoothPlayhead && hasPlayStepEstimate && playStepMs > 0.0)
    {
        const double elapsed = juce::Time::getMillisecondCounterHiRes() - playStepStartMs;
        const double fraction = juce::jlimit(-0.12, 1.45, elapsed / playStepMs);
...
float GgdDrumGrid::getPlayheadContentX() const
{
    const float tick = interpolatedPlayTick();
    return tick >= 0.0f ? xForTick(tick) : -1.0f;
}
...
void GgdDrumGrid::paint(juce::Graphics& g)
{
    ...
    // The cell is the visual unit in Draw mode. Grid lines mark its edges and
    // the hit itself sits in the middle, which reads much more naturally as a
    // step sequencer than pinning every hit body against its left timing line.
    if (toolMode == ToolMode::draw && hoverCanonicalRow >= 0 && hoverTick >= 0)
    {
        ...
        if (y >= 0)
        {
            ...
            const float w = juce::jmax(5.0f, cellWidthPixels());
    ...
    if (const auto* p = pattern())
    {
        for (int index = 0; index < p->getEventCount(); ++index)
        {
            ...
            const float slotPixels = cellWidthPixels();
            const float hitW = juce::jlimit(8.0f, 30.0f, slotPixels * 0.62f);
            const float v = static_cast<float>(event->velocity) / 127.0f;
            const float hitH = 7.0f + 13.0f * v;
            const float cx = visualHitCenterX(event->tick);
            const float cy = y + rowHeight * 0.5f;
            juce::Rectangle<float> hit(cx - hitW * 0.5f,
                                       cy - hitH * 0.5f,
                                       hitW, hitH);
    ...
    if (dragMode == DragMode::move && (dragDeltaTicks != 0 || dragDeltaRows != 0))
    {
        for (const auto& state : dragSelectionStates)
        {
            ...
            if (y < 0 || tick < 0 || tick >= length)
                continue;
            const float cx = visualHitCenterX(tick);
            g.setColour(gc(GgdThemeRole::warm).withAlpha(0.72f));
            g.fillRoundedRectangle(cx - 5.0f, y + rowHeight * 0.5f - 5.0f,
                                   10.0f, 10.0f, 3.0f);
        }
    }
    ...
    if (smoothTick >= 0.0f)
    {
        ...
        if (playheadGlow)
        {
            // The glow deliberately trails to the right: it gives the playhead
            // a small sense of forward energy without blurring the timing edge.
            juce::ColourGradient glow(
                playhead.withAlpha(0.18f), playX, 0.0f,
                playhead.withAlpha(0.0f), playX + 52.0f, 0.0f,
                false);
            g.setGradientFill(glow);
            g.fillRect(juce::Rectangle<float>(
                playX, 0.0f, 52.0f, static_cast<float>(getHeight())));
        ...
        g.setColour(playhead.withAlpha(0.98f));
        g.fillRect(juce::Rectangle<float>(
            playX - 0.75f, 0.0f, 1.5f, static_cast<float>(getHeight())));
    ...
    // Sticky labels are painted last so the moving playhead/glow never leaks
    // through the articulation column while the viewport is following playback.
    g.setColour(gc(GgdThemeRole::panel));
...
GgdDrumGrid::EventRef GgdDrumGrid::eventAt(float x, float y) const
{
    ...
    const float hitW = juce::jlimit(8.0f, 30.0f, cellWidthPixels() * 0.62f);
    ...
    for (int i = 0; i < p->getEventCount(); ++i)
    {
        ...
        const float centre = visualHitCenterX(event->tick);
        const float left = centre - hitW * 0.5f - 4.0f;
        const float right = centre + hitW * 0.5f + 4.0f;
        if (x < left || x > right)
            continue;

        const float distance = std::abs(x - centre);
        if (distance < best)
        {
            best = distance;
            bestTick = event->tick;
...
void GgdDrumGrid::mouseDown(const juce::MouseEvent& e)
{
    ...
    if (e.position.x < stickyX + nameWidth)
        return;
    ...
    if (toolMode == ToolMode::draw)
    {
        ...
        // the currently selected cell happens to be empty.
        ...
        const int tick = cellTickForX(e.position.x);
...
void GgdDrumGrid::mouseDrag(const juce::MouseEvent& e)
{
    ...
    const int row = canonicalRowAtY(e.position.y);
    if (row < 0 || e.position.x < currentViewX() + nameWidth)
        return;
    ...
    if (dragEvent.tick == -3)
    {
        ...
        for (int i = p->getEventCount() - 1; i >= 0; --i)
        {
            ...
            if (event == nullptr || event->row != storage)
                continue;
            const float centre = visualHitCenterX(event->tick);
            const float left = centre - hitW * 0.5f - 3.0f;
            const float right = centre + hitW * 0.5f + 3.0f;
            if (right >= lo && left <= hi)
            {
    ...
    const int target = cellTickForX(e.position.x);
...
void GgdDrumGrid::mouseMove(const juce::MouseEvent& e)
{
    const int row = canonicalRowAtY(e.position.y);
    const int stickyX = currentViewX();
    const bool overSticky = e.position.x >= stickyX && e.position.x < stickyX + nameWidth;
    const int tick = row >= 0 && !overSticky
        ? cellTickForX(e.position.x) : -1;
